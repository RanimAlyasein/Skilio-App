...
- [Data Model](#data-model--12-entities)
...
├── skilio-backend/
│   ├── app/
│   │   ├── api/              # Route handlers (auth, children, scenarios, modules, badges, lessons)
...
| `DELETE /api/children/{id}` | Children | Delete child profile |
...
| `GET /api/children/{id}/attempts` | Children | Full scenario attempt history |
| `GET /api/children/{id}/summary` | Children | XP and overall stats summary |
| `GET /api/modules` | Modules | List skill worlds (with optional age filter) |
| `GET /api/modules/{id}` | Modules | World detail with lessons |
| `GET /api/lessons/{id}` | Lessons | Lesson detail with scenario entry point |
| `POST /api/scenarios/start` | Scenarios | Begin a new scenario attempt |
| `POST /api/scenarios/choose` | Scenarios | Submit a choice in the current node |
| `GET /api/scenarios/history/{child_id}` | Scenarios | Full decision history for a child |
| `GET /api/scenarios/resume/{attempt_id}` | Scenarios | Resume a paused scenario |
| `GET /api/scenarios/{id}` | Scenarios | Scenario node detail |
| `GET /api/badges` | Badges | Full badge catalogue |
| `GET /api/badges/{id}` | Badges | Single badge detail |
| `GET /health` | Health | Server and database health check |
...
FastAPI (Python 3.11 + uvicorn)
    │  6 routers · 24 endpoints · CORS · Pydantic v2 validation · Depends injection
    │  Rate limiting · Size limits · Request ID middleware · Slow-request logging
    ▼
SQLAlchemy ORM 2.0 + Alembic
    │  12 entities · Mapped[] type annotations · Relationships · Migrations
    ▼
Database
